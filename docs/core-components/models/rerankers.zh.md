...
Rerankers 提供 Heta 与重排序模型服务通信的统一入口。它属于 Models 组件的一部分，负责根据用户查询重新评估候选文档、chunk 或图谱证据的相关性。
...
`model_name` 遵循 LiteLLM 的 rerank 命名规则。LiteLLM 当前支持 Cohere、Together AI、Azure AI、DeepInfra、Nvidia NIM、Infinity、Fireworks AI、Voyage AI、watsonx.ai 等 rerank provider。
...
| `RerankModelProtocol` | Rerank 能力协议，用于 Recipe、检索引擎和自定义模型的类型约束。 |
| `RerankRequest` | 一次重排序请求，包含 query、候选文档、调用参数和追踪信息。 |
| `RerankOptions` | 单次请求参数，例如返回数量、是否返回文档和服务方专有参数。 |
| `RerankResult` | 重排序结果，包含有序的 `RerankItem` 列表、原始响应和追踪信息。 |
| `RerankItem` | 一个候选文档的排序结果，包含原始索引、相关性分数和可选文档文本。 |

`RerankModelProtocol` 是结构化协议，不要求用户继承某个父类。自定义 rerank 模型只要实现 `rerank` 和 `rerank_many`，就可以被后续检索引擎接收。
...
| 字段 | 说明 |
| --- | --- |
| `rankings` | 有序的 `RerankItem` 列表。顺序由 rerank 模型决定，通常从最相关到最不相关。 |
| `model_name` | 当前 `RerankModel` 配置的模型名。 |
| `trace_context` | 请求携带的追踪信息。 |
| `raw_response` | LiteLLM 返回的原始响应字典。 |

`RerankItem.index` 是原始候选文档在 `RerankRequest.documents` 中的位置。检索引擎应使用这个索引回到原始 `QueryResult`，而不是把 reranker 当成新的内容来源。
